...
def build_wds_dataset(dataset_name, transform, split="test", data_dir="root", cache_dir=None):
    ...
    def read_txt(fname):
        # If cache_dir is provided and we have a URL, try to read from cache first
        if cache_dir and "://" in fname:
            # Extract dataset name from the URL
            if "wds_" in fname:
                ds_name = fname.split("wds_")[1].split("/")[0]
                # Construct local cache path
                if "/test/" in fname or f"/{split}/" in fname:
                    cache_fname = os.path.join(cache_dir, ds_name, split, os.path.basename(fname))
                else:
                    cache_fname = os.path.join(cache_dir, ds_name, os.path.basename(fname))
                
                # Try to read from cache first
                if os.path.exists(cache_fname):
                    print(f"✓ Reading from cache: {cache_fname}")
                    with open(cache_fname, "r") as file:
                        return file.read()
                else:
                    print(f"✗ Cache file not found: {cache_fname}")
        
        # Original logic: download from URL or read from local file
        if "://" in fname:
            stream = os.popen("curl -L -s --fail '%s'" % fname, "r")
            value = stream.read()
            if stream.close():
                raise FileNotFoundError(f"Failed to retrieve data from {fname}")
        else:
    ...
    if data_dir.startswith("https://huggingface.co/datasets"):
        # Check if we can use local cache instead
        if cache_dir and os.path.exists(cache_dir):
            # Extract the actual dataset name from the URL template
            # data_dir might be: "https://huggingface.co/datasets/clip-benchmark/wds_{dataset_cleaned}/tree/main"
            # dataset_name might be: "vtab/cifar10"
            
            # Clean the dataset name for local filesystem
            ds_name_cleaned = dataset_name.replace("/", "-")
            local_cache_path = os.path.join(cache_dir, ds_name_cleaned)
            nshards_local = os.path.join(local_cache_path, split, "nshards.txt")
            
            print(f"Checking for local cache at: {local_cache_path}")
            
            if os.path.exists(nshards_local):
                print(f"✓ Using LOCAL cache: {local_cache_path}")
                metadata_dir = tardata_dir = local_cache_path
            else:
                print(f"✗ Local cache not found, constructing URL")
                # Format: https://huggingface.co/datasets/<USERNAME>/<REPO>/tree/<BRANCH>
                # Replace {dataset_cleaned} placeholder with actual cleaned name
                data_dir = data_dir.format(dataset_cleaned=ds_name_cleaned)
            ...
            # No cache or cache doesn't exist
            ds_name_cleaned = dataset_name.replace("/", "-")
            data_dir = data_dir.format(dataset_cleaned=ds_name_cleaned)
            *split_url_head, _, url_path = data_dir.split("/", 7)
            url_head = "/".join(split_url_head)
    ...
    print(f"tardata_dir: {tardata_dir}")
    
    # Get number of shards
    nshards_fname = os.path.join(metadata_dir, split, "nshards.txt")
    print(f"Reading nshards from: {nshards_fname}")
    nshards = int(read_txt(nshards_fname))
    print(f"Number of shards: {nshards}")
    ...
    dataset = (
        wds.WebDataset(filepattern, handler=wds.handlers.warn_and_continue, cache_dir=None)
