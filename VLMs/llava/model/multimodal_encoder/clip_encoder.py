...
class CLIPVisionTower(nn.Module):
    ...
    def load_model(self, non_llava=False, pretrained_ckpt=None, device='cuda'):
        ...
        if non_llava:
            ...
            second_model, _, image_processor = open_clip.create_model_and_transforms('ViT-L-14', pretrained='openai')
            self.full_clip = second_model
            ...
            # prima del taglio
            self.proj = vision_model.proj
            self.last_block = vision_model.transformer.resblocks[-1]
            
            vision_model.transformer.resblocks = vision_model.transformer.resblocks[:-1]
            print("removing last layer of vision model, saving it as self.last_block")
            print("The last layer is {}".format(self.last_block))
            ...
            self.visual_backbone = vision_model  # per accedere ai layer LN/post/proj
    ...
    def forward_full_clip(self, images):
        """
        Esegue tutto il vision transformer ViT-L/14 con l'ultimo blocco e proiezione,
        restituendo l'embedding CLS normalizzato come in CLIP originale.
        """
        assert self.non_llava, "forward_full_clip disponibile solo in modalità open_clip"
        ...
        with torch.set_grad_enabled(True):
            ...
            if self.proj is not None:
                cls = cls @ self.proj
                print("Applied projection in forward_full_clip")
            else:
                print("Warning: projection is None in forward_full_clip")

            # normalizza come in CLIP
