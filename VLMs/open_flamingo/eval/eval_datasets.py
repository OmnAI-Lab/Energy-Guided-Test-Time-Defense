...
class VQADataset(Dataset):
    def __init__(
        ...
        if self.dataset_name in {"vqav2", "ok_vqa"}:
            ...
            # If we are loading .pt adversarial images, we do NOT enforce COCO split naming
            if not is_tensor:
...
class TensorCaptionDataset(CaptionDataset):
    ...
    def get_from_id(self, image_id):
        if self.dataset_name == "coco":
            filename = f"{str(image_id).zfill(12)}.pt"
            return torch.load(os.path.join(self.image_val_dir_path, filename))
            ...
            filename = f"COCO_{self.annotations_split}2014_{str(image_id).zfill(12)}.pt"
            return torch.load(os.path.join(self.image_val_dir_path, filename))
            ...
            raise ValueError(f"Dataset {self.dataset_name} non supportato per get_from_id().")

    def __getitem__(self, idx):
        ann = self.annotations[idx]

        if self.dataset_name == "coco":
            image_filename = ann["filename"].replace(".jpg", ".pt")
            image = torch.load(os.path.join(
                self.image_train_dir_path if ann["filepath"] == "train2014" else self.image_val_dir_path,
                image_filename
            ))
            ...
            image_filename = ann["filename"].replace(".jpg", ".pt").replace(".png", ".pt")
            image = torch.load(os.path.join(self.image_val_dir_path, image_filename))
            caption = ann["sentences"][0]["raw"]
            image_id = ann["imgid"]

        elif self.dataset_name == "vqa":
            image_id_num = ann["image_id"]
            filename = f"COCO_{ann['split']}2014_{str(image_id_num).zfill(12)}.pt"
            image = torch.load(os.path.join(self.image_val_dir_path, filename))
            caption = ann["question"]
            image_id = image_id_num

        elif self.dataset_name == "textvqa":
            image_id_num = ann["image_id"]
            filename = f"{image_id_num}.pt"
            image = torch.load(os.path.join(self.image_val_dir_path, filename))
            caption = ann["question"]
            image_id = image_id_num
