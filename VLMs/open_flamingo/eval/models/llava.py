...
class EvalModelLLAVA(BaseEvalModel):
    ...
    def __init__(self, model_args):
        ...
        print(f"Loading LLaVA with args {model_args}")
        print(f'Using LLaVA with CLIP {model_args["vision_encoder_pretrained"]}')
    ...
    def get_outputs(
        ...
        **kwargs,
        ...
        output_ids = self.model.generate(
            input_ids,
            images=batch_images.to(dtype=self.cast_dtype, device=self.device, non_blocking=True),
    ...
    def _prepare_text(self, convs):
        ...
        input_ids = torch.stack(input_ids, dim=0).to(device=self.device, non_blocking=True)
        return input_ids
