...
ID_RUN="PURIFY_CLEANDATA_OVERNIGHT_10ITERS_$(date +%Y%m%d_%H%M%S)_$(uuidgen | cut -c1-8)"
...
for ((i=0; i<${#VISION_ENCODER_LIST[@]}; i++)); do
  ...
  for ((j=0; j<${#EVAL_FLAGS[@]}; j++)); do
    ...
    sbatch \
      ...
      --output="logs/${dataset_name}_ve-${ve_base}_cf-${cf}_%j.out" \
      --error="logs/${dataset_name}_ve-${ve_base}_cf-${cf}_%j.err" \
      "$SBATCH_TEMPLATE"
